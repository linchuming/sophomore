...
<?php
	...
	{
		$selectSQL = "select id from students where id=" . GetSQLValueString($_POST['username'],"int");
		...
		if($info!=false)
		{
			echo "<script type='text/javascript'> alert('用户名已存在！'); self.location = 'register.php'; </script>";
			return;
		}
		$md5_pass = md5($_POST['password']);
		$insertSQL = sprintf("insert into students(id,name,password,checked) values(%d,%s,%s,0)",
