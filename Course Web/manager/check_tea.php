...
<?php
	...
	if(isset($_GET['check']))
	{
		$insertSQL = sprintf("select checked from teachers where id=%d",GetSQLValueString($_GET['check'],"int"));
		...
		$insertSQL = sprintf("update teachers set checked=1 where id=%d",GetSQLValueString($_GET['check'],"int"));
		$result = mysql_query($insertSQL,$MySql);
	}
...
<div id="center">
  <table align="center" width="600" border="1" cellspacing="0px"  style="border-collapse:collapse">
    ...
    <?php
    	$result = mysql_query("select id,name,faculty from teachers where checked=0",$MySql);
