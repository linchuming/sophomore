...
<?php
	...
	{
		$insertSQL = sprintf("update teachers set name=%s,faculty=%s where id=%d",
	...
	$insertSQL = sprintf("select name,faculty from teachers where id=%d",GetSQLValueString($id,"int"));
	$result = mysql_query($insertSQL,$MySql);
  ...
  <form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']."?id=".$id; ?>">
    ...
    <p>院系：
      <select name="faculty" id="faculty">
        <option value="计算机科学与技术" <?php if($info['faculty'] == "计算机科学与技术") echo "selected";?>>计算机科学与技术</option>
        <option value="微电子科学与技术" <?php if($info['faculty'] == "微电子科学与技术") echo "selected";?>>微电子科学与技术</option>
        <option value="中文系" <?php if($info['faculty'] == "中文系") echo "selected";?>>中文系</option>
      </select>
    </p>
